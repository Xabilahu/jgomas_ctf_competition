...
+!get_agent_to_aim
    ...
    /*?startCuresPrepared(C)
    if(C==0){
        ?my_position(X, Y, Z);
        if(X < 130){
            !add_task(task("TASK_GOTO_POSITION", "M1", pos(130, 0, Z), ""));
	        -+state(standing);
        }else{
            -
        }
...
+!init
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR init GOES HERE.")};
    ...
    +tasks([]);
    ?my_position(X,Y,Z);
    //!add_task(task(10,"TASK_GOTO_POSITION", "M1", pos(100, 0, Z), ""));
    !add_task(task(9,"TASK_GIVE_MEDICPAKS", "M1", pos(100, 0, Z), ""));
    //!add_task(task(10,"TASK_GOTO_POSITION", "M1", pos(100, 0, Z+5), ""));
    !add_task(task(9,"TASK_GIVE_MEDICPAKS", "M1", pos(100, 0, Z+5), "")).
