...
+!get_agent_to_aim
    ...
    if((X>=(Xa-1) & X<=(Xa+1)) & (Z>=(Za-1) & Z<=(Za+1))){
        !add_task(task(5000,"TASK_GOTO_POSITION", "M1", pos(X, 0,Z+4), ""));
...
+!init
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR init GOES HERE.")};
    ...
    +arrived_pos(95,225); 
    ?my_position(X,Y,Z);
    !add_task(task(4000,"TASK_WALKING_PATH","M1", pos(95, 0,228), ""));
    !add_task(task(5000,"TASK_GOTO_POSITION", "M1", pos(95, 0,225), "")).
